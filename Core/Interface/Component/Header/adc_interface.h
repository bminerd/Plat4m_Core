 ...
 * @brief ADC ID type.
 */
typedef struct _adc_id_t_
{
    const adc_t adc;
    const generic_data_t private[sizeof(adc_info_t) / sizeof(private_data_t)];
} adc_id_t;

/**
 * @brief Function type that sets the given ADC enabled or disabled.
 * @param adc ADC to access.
 * @param enabled Flag that indicates if the ADC should be enabled or disabled.
 */
typedef void adc_driver_set_enabled_f(const adc_t* adc, const bool enabled);

/**
 * @brief Function type that configures the given ADC.
 * @param adc ADC to access.
 * @param config ADC configuration.
 */
typedef void adc_driver_configure_f(const adc_t* adc,
                                    const adc_config_t* config);

/**
 * @brief Function type that reads a value from the given ADC.
 * @param adc ADC to access.
 * @param voltage Voltage read from the ADC.
 * @return ADC error.
 */
typedef void adc_driver_read_f(const adc_t* adc, float* voltage);
...
typedef struct _adc_driver_t_
{
    ...
    adc_driver_configure_f* configure;
...
/**
 * @brief Adds the given ADC ID.
 * @param adc ADC ID to add.
 * @return ADC error.
 */
