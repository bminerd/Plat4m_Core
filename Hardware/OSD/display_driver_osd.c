 ...
 */
static display_error_e setEnabled(const bool enabled);
 ...
 * @return Display error.
 */
static display_error_e writeData(const display_data_definition_t* definition,
 ...
 * @return Display error.
 */
static display_error_e writeText(const display_data_definition_t* definition,
 ...
 * @return Display error.
 */
static display_error_e clear(void);
 ...
 * @return Display error.
 */
static display_error_e setBrightness(const uint8_t brightness);

/**
 * TODO Comment!
 */
static display_error_e updateDisplay(const display_data_t* data);

/**
 * TODO Comment!
 */
static display_error_e slideHoriz(const transition_direction_e direction,
                       const int areaStart,
                       const int areaEnd,
                       const int areaSize,
                       const int areaWidth,
 ...
 * TODO Comment!
 */
static display_error_e slideVert(const transition_direction_e direction,
                      const int areaStart,
                      const int areaEnd,
                      const int areaSize,
                      const int areaWidth,
...
extern void displayDriverInit(void)
{
    ...
    gpio_t gpios[] =
    {
        // RST_N
        {
            .gpio           = GPIO_OLED_RST_N,
            .hardware.port  = GPIO_PORT_E,
            .hardware.pin   = GPIO_PIN_8
        },
        // VCC_EN
        {
            .gpio           = GPIO_OLED_VCC_EN,
            .hardware.port  = GPIO_PORT_E,
            .hardware.pin   = GPIO_PIN_9
        },
        // PWR_EN
        {
            .gpio           = GPIO_OLED_PWR_EN,
            .hardware.port  = GPIO_PORT_E,
            .hardware.pin   = GPIO_PIN_10
        }
    };

    gpioAddGpios(gpios, ARRAY_SIZE(gpios));
    ...
    gpioSetEnabled(GPIO_OLED_RST_N, true);
    gpioSetEnabled(GPIO_OLED_VCC_EN, true);
    gpioSetEnabled(GPIO_OLED_PWR_EN, true);
    gpioConfigure(GPIO_OLED_RST_N, &gpioConfig);
    gpioConfigure(GPIO_OLED_VCC_EN, &gpioConfig);
    gpioConfigure(GPIO_OLED_PWR_EN, &gpioConfig);

    i2c_device_t i2cDevice =
    {
        .device     = I2C_DEVICE_DISPLAY,
        .i2c        = I2C_1,
...
//------------------------------------------------------------------------------
extern display_error_e setEnabled(const bool enabled)
{
    int i = 0;
    byte_array_t data;
...
#ifdef OLED_WHITE
        // Enable V_DD
        gpioSetLevel(GPIO_OLED_PWR_EN, GPIO_LEVEL_HIGH);
        // Wait for V_DD to stabilize
        systemDelayMs(10);
        // Pull RST low
        gpioSetLevel(GPIO_OLED_RST_N, GPIO_LEVEL_LOW);
        systemDelayMs(1);
        // Push RST high
        gpioSetLevel(GPIO_OLED_RST_N, GPIO_LEVEL_HIGH);
        systemDelayMs(1);

//        gpioSetLevel(GPIO_OLED_VCC_EN, GPIO_LEVEL_HIGH);
//        systemDelayMs(100);
        ...
        i2c_error_e error = i2cMasterTx(I2C_DEVICE_DISPLAY, &data);
        ...
        if (i2cMasterTx(I2C_DEVICE_DISPLAY, &data) != I2C_ERROR_NONE)
        {
            return DISPLAY_ERROR_COMMUNICATION;
        }

        display_error_e displayError = clear();
    ...
    else
    {
        ...
        // Disable V_CC
        gpioSetLevel(GPIO_OLED_VCC_EN, GPIO_LEVEL_LOW);
        systemDelayMs(100);
        // Disable V_DD
        gpioSetLevel(GPIO_OLED_PWR_EN, GPIO_LEVEL_LOW);
    }
...
//------------------------------------------------------------------------------
extern display_error_e writeData(const display_data_definition_t* definition,
                                 const data_array_t* data)
{
    ...
    return updateDisplay(&displayData);
}

//------------------------------------------------------------------------------
extern display_error_e writeText(const display_data_definition_t* definition,
...
//        {
//            dataIndex = (sections[section].x * 2) + 1;
//
//            break;
//        }
//        case DISPLAY_ALIGNMENT_CENTER:
//        {
//            dataIndex = ((sections[section].x +
//                             (sections[section].width / 2) -
//                          (size / 2)) * 2) + 1;
//
//            break;
//        }
...
//        {
//            dataIndex = ((sections[section].x + sections[section].width -
//                          size) * 2) + 1;
//
//            break;
//        }
...
//------------------------------------------------------------------------------
extern display_error_e clear(void)
{
    int i = 0;
...
//------------------------------------------------------------------------------
extern display_error_e setBrightness(const uint8_t brightness)
{
    int i = 0;
...
static display_error_e updateDisplay(const display_data_t* data)
{
    ...
    {
        case DISPLAY_TRANSITION_NONE:
        {
            memcpy(bytes, newBytes, ARRAY_SIZE(newBytes));

            if (i2cMasterTx(I2C_DEVICE_DISPLAY, &i2cData) != I2C_ERROR_NONE)
            {
                return DISPLAY_ERROR_COMMUNICATION;
            }
