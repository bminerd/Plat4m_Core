...
//------------------------------------------------------------------------------
void interruptDriverStm32f2xxInit(void)
{
    ...
    interruptSetDriver(&driver, &intHandler);
    ...
    // Test line
    gpioInitId(&gpioIntTest);
    gpioSetEnabled(&gpioIntTest, true);
    gpioConfigure(&gpioIntTest, &gpioConfig);
}
...
static void configure(const interrupt_e interrupt,
                      const interrupt_config_t* config)
{
    SYSCFG_EXTILineConfig(config->gpio->gpio.port,
                          config->gpio->gpio.pin);

    EXTI_InitTypeDef extiInit =
    {
        .EXTI_Line      = lineMap[config->gpio->gpio.pin],
...
{
    gpioSetLevel(&gpioIntTest, GPIO_LEVEL_HIGH);
    ...
    gpioSetLevel(&gpioIntTest, GPIO_LEVEL_LOW);
}
