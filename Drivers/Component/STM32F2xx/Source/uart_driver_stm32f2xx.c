 ...
 */
static const uart_gpio_t gpioMap[UART_COUNT][UART_GPIO_MAP_COUNT] =
{
    /// UART_1
    {
        /// UART_GPIO_MAP_1
...
//------------------------------------------------------------------------------
void uartDriverStm32f2xxInit(void)
{
    // plat4m
    //
    ...
    uartSetDriver(&driver, &intHandler);
}
...
static void configure(const uart_e uart, const uart_config_t* config)
{
    ...
    gpioHardwareSetEnabled(&(gpioMap[uart][config->map].rx), true);
